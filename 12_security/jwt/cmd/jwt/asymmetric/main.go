...
func execute() error {
	...
	token, err := assymetric.Encode(data, privateKey)
	if err != nil {
		return err
	}
	...
	verified, err := assymetric.Verify(token, publicKey)
	if err != nil {
		return err
	}
	...
	var decoded *Data
	err = assymetric.Decode(token, &decoded)
	if err != nil {
		return err
	}
