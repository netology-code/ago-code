...
func Verify(token string, key []byte) (ok bool, err error) {
	...
	err = verifySignatureEncoded(headerEncoded, payloadEncoded, signatureEncoded, key)
	if err != nil {
		return false, err
	}
	return true, nil
...
func verifySignatureEncoded(headerEncoded string, payloadEncoded string, signatureEncoded string, key []byte) error {
	block, _ := pem.Decode(key)
	if block == nil {
		return fmt.Errorf("can't decode block")
	}
