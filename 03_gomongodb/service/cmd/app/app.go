...
func (s *Server) Search(writer http.ResponseWriter, request *http.Request) {
	rating, err := strconv.ParseFloat(request.FormValue("min_rating"), 64)
	if err != nil {
		log.Print(err)
		writer.WriteHeader(http.StatusBadRequest)
		return
	...
	cursor, err := s.db.Collection("orders").Find(request.Context(), bson.D{{"film.rating", bson.D{{"$gt", rating}}}})
	if err != nil {
		log.Print(err)
		writer.WriteHeader(http.StatusInternalServerError)
		return
...
func (s Server) Save(writer http.ResponseWriter, request *http.Request) {
	...
	if order.ID == primitive.NilObjectID {
		...
		result, err := s.db.Collection("orders").InsertOne(request.Context(), &order)
		if err != nil {
			log.Print(err)
			writer.WriteHeader(http.StatusInternalServerError)
			return
		...
		result, err := s.db.Collection("orders").UpdateOne(request.Context(), bson.M{"_id": order.ID}, bson.D{
			{"$set", bson.D{
				{"start", order.Start},
				{"price", order.Price},
			}},
