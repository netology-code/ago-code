...
func execute(brokerURL string, paymentTopic string, transactionsTopic string, group string) error {
	...
	consumerGroup, producer, err := waitForKafka(brokerURL, group, config)
	if err != nil {
		return err
	}
...
func waitForKafka(brokerURL string, group string, config *sarama.Config) (sarama.ConsumerGroup, sarama.SyncProducer, error) {
	for {
		select {
...
func (h *Handler) ConsumeClaim(session sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
	...
	for message := range claim.Messages() {
		...
		log.Printf("sending message to topic: %s", h.TransactionsTopic)
